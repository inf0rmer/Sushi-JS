/*
 * Sushi.gfx - 3D CSS transforms for awesome transitions. Based on Zepto GFX (https://github.com/gobhi/zepto-gfx)
 *
 */
 define('sushi.gfx',
 	// Module dependencies
 	 ...
 	 * @class gfx
 	 */
 	function(Sushi, $, collection, support) {
        var defaults = {
			duration: 400,
			easing: ''
		},
		...
		$.fn.gfxPopIn = function (options, cb) {
			...
			$that.transform({
				...
				opacity: '0',
				display: 'block'
			}).animate({scale: '1', opacity: '1'}, opts.duration, opts.easing, cb);
		};
		...
		$.fn.gfxPopOut = function (options, cb) {
			...
			$that.transform({
				...
				scale: '1',
				opacity: '1'
			}).animate({opacity: 0, scale: opts.scale}, opts.duration, opts.easing, function () {
		...
		$.fn.gfxFadeIn = function (options, cb) {
			...
			$that.css({opacity: 0}).show().animate({opacity: 1}, opts.duration, opts.easing, cb);
		};
		...
		$.fn.gfxFadeOut = function (options, cb) {
			...
			$that.css({opacity: 1}).animate({opacity: 0}, opts.duration, opts.easing,
				function () {
		...
		$.fn.gfxShake = function (options, cb) {
			var $that = $(this),
				...
				second = function () {
					$that.animate({translateX: '-' + distance + 'px'}, opts.duration, opts.easing, third);
				},
				first = function () {
					$that.animate({translateX: distance + 'px'}, opts.duration, opts.easing, second);
				};
			...
			$that.animate({translateX: '-' + distance + 'px'}, opts.duration, opts.easing, first);
		};
		...
		$.fn.gfxBlip = function (options, cb) {
			var $that = $(this),
				opts = Sushi.extend({}, defaults, options || {}),
				first = function () {
					$that.animate({scale: 1}, opts.duration, opts.easing, cb);
				};

			opts.scale = opts.scale || 1.15;

			$that.animate({scale: opts.scale}, opts.duration, opts.easing, first);
		};
		...
		$.fn.gfxExplodeIn = function (options, cb) {
			...
			$that.transform({scale: opts.scale, opacity: 0, display: 'block'})
				.animate({scale: 1, opacity: 1}, opts.duration, opts.easing, cb);
		};
		...
		$.fn.gfxFlipIn = function (options, cb) {
			...
			$that.transform({rotateY: '180deg', scale: 0.8, display: 'block'}).animate({rotateY: 0, scale: 1}, opts.duration, opts.easing, cb);
		};
		...
		$.fn.gfxFlipOut = function (options, cb) {
			...
			$that.transform({rotateY: 0, scale: 1})
				.animate({rotateY: '-180deg', scale: 0.8}, opts.duration, opts.easing, c);
		};
		...
		$.fn.gfxRotateOut = function (options, cb) {
			...
			$that.transform({rotateY: 0}).animate({rotateY: '-180deg'}, opts.duration, opts.easing, c);
		};
		...
		$.fn.gfxSlideOut = function (options, cb) {
			...
			if (support.transform3D) {
				$that.show().animate({translate3d: distance + ',0,0', opacity: opacity}, opts.duration, opts.easing,
					function () {
		...
		$.fn.gfxSlideIn = function (options, cb) {
			...
			if (support.transform3D) {
				$that.transform({translate3d: distance + ',0,0', opacity: opacity}).show().animate({translate3d: '0,0,0', opacity: 1}, opts.duration, opts.easing, cb);
			} else {
				$that.show();
				cb && cb();
			}

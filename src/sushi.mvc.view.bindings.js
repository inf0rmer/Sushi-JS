 ...
 *
 */
 define('sushi.mvc.view.bindings',
 	// Module dependencies
 	[
 		'sushi.core',
 		'sushi.utils',
 	 ...
 	 *
 	 * @namespace Sushi
 	 * @class mvc.view.bindings
 	 */
 	function(Sushi, utils, collection, View, SushiError) {
 	
 		var bindingSplitter = /^(\S+)\s*(.*)$/,
			identityTransformer = function(value) {
				return value;
			},
			...
			originalDealloc = View.prototype.dealloc;
        
        Sushi.extend(View.prototype, {
        	...
        	bindModel: function(bindings) {
				...
				collection.each(bindings, function(attribute, binding) {
					...
					var match = binding.match(bindingSplitter),
						...
						binder = View.Binders[property] || View.Binders['__attr__'],
					...
					if (!utils.isArray(accessors.get))
					...
					var setTrigger = 'change:' + attribute[0],
						...
						getTrigger = collection.reduce(accessors.get[0].split(' '), function(memo, event) {
					...
					var set = utils.bind(function(model, value, options) {
					...
					var get = utils.bind(function(event) {
					...
					if (accessors.set) {
						this.model.bind(setTrigger, set);
			...
			unbindModel: function() {
				...
				collection.each(this._bindings, function(binding) {
					if (binding.get[1])
						this.$el.unbind(binding.getTrigger);
	
					if (binding.set)
						this.model.unbind(binding.setTrigger, binding.set);
			...
			dealloc: function() {
				this.unbindModel();
				originalDealloc.apply(this, arguments);
			}
        });
